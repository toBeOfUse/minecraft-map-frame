// import { Map, PointOfInterest, Coords, Line, mod, getEdgeLength } from "./Types";
...
//         const mapsToCheck = this.maps.size < otherIsland.maps.size ? this.maps : otherIsland.maps;
//         const checkingAgainst =
//             this.maps.size < otherIsland.maps.size ? otherIsland.localMapIndex : this.localMapIndex;
...
//         const ourCenterMapX = Math.floor(ourXCenter / this.edgeLength) * this.edgeLength;
//         const ourCenterMapY = Math.floor(ourYCenter / this.edgeLength) * this.edgeLength;

//         console.assert(
//             this.localMapIndex.has(Island.coordsToID({ x: ourCenterMapX, y: ourCenterMapY }))
//         );
...
//             Math.floor(theirXCenter / otherIsland.edgeLength) * otherIsland.edgeLength;
//         const theirCenterMapY =
//             Math.floor(theirYCenter / otherIsland.edgeLength) * otherIsland.edgeLength;
...
//         const shapeCorners = sortedCorners.filter(c => c.angle !== CornerType.Concave);
//         const shapeLines = [];
//         for (let i = 0; i < shapeCorners.length; i++) {
//             const cornerFrom = shapeCorners[i];
//             const cornerTo = shapeCorners[(i + 1) % shapeCorners.length];
//             shapeLines.push(new Line(cornerFrom.x, cornerFrom.y, cornerTo.x, cornerTo.y));
//         }
